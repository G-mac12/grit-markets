...
export const START_GUIDES: StartGuide[] = [
  {
    ...
    intro:
      "Before any automated trading software can run, it needs two things: a broker and a trading account. A broker is a regulated company that connects your orders to the currency market; a trading account is your named record with that broker, holding your balance and your trades. An Expert Advisor, usually shortened to EA, is a piece of software that places and manages trades inside the MetaTrader 5 platform automatically. Not every broker is equally suitable for running one, so this guide gives you a simple framework for judging any broker, then walks through opening a demo account with IC Markets as the worked example. [OWNER INPUT: confirm IC Markets as featured broker; if an introducing-broker or affiliate link is used, the relationship must be disclosed on this page]. A demo account uses virtual money on real market prices, so everything in this path can be completed without risking a penny. A live account, funded with real money, is a final and entirely optional step for later.",
    steps: [
      {
