-- Grit Markets — DASHBOARD TABLE RESET
-- Use when the Phase 2b tables exist with the WRONG SHAPE (e.g. created by
-- a Supabase sample/quickstart): drops ONLY the dashboard tables below and
-- recreates them correctly. Profiles, subscriptions, licenses and your
-- login are NOT touched. Any demo data in the dropped tables is lost
-- (re-run seed_demo_data.sql afterwards to restore it).
...
drop table if exists public.skim_ledger cascade;
drop table if exists public.skim_rules cascade;
