-- Grit Markets Phase 2b: telemetry, account links, analytics, safety buffer,
-- strategy settings sync, alerts, email preferences. RLS on every table;
-- writes happen via the service role in server routes only.
...
-- ------------------------------------------------------------- licenses
-- per-license telemetry secret (AES-256-GCM encrypted with a server env key;
-- plaintext shown once at issue/regeneration, never logged)
...
create index account_links_user_idx on public.account_links (user_id);
...
create index telemetry_snapshots_acct_ts_idx
...
create index trades_acct_close_idx on public.trades (account_link_id, close_time desc);
...
-- -------------------------------------------------------------- skim_rules
create table public.skim_rules (
  id uuid primary key default gen_random_uuid (),
  account_link_id uuid not null references public.account_links (id) on delete cascade,
...
-- ------------------------------------------------------------- skim_ledger
create table public.skim_ledger (
  id uuid primary key default gen_random_uuid (),
  account_link_id uuid not null references public.account_links (id) on delete cascade,
  created_at timestamptz not null default now(),
...
create index skim_ledger_acct_idx on public.skim_ledger (account_link_id, created_at desc);
...
create table public.email_preferences (
  ...
  weekly_newsletter boolean not null default false, -- consent: unticked default
...
-- license_events: allow 'telemetry' event type
alter table public.license_events drop constraint license_events_event_check;
...
-- Retention notes (enforced by cron):
--   telemetry_snapshots: 24 months rolling
--   license_events (incl. IP): 90 days
--   deleted accounts: purged within 30 days of grace expiry
